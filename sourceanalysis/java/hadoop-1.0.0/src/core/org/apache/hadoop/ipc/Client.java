...
public class Client {
  ...
  private class Connection extends Thread {
    ...
    private synchronized void setupConnection() throws IOException {
      ...
      while (true) {
        try {
          this.socket = socketFactory.createSocket();//底层仍然是socket
          ...
          NetUtils.connect(this.socket, server, 20000);//设置超时时间为20秒
          if (rpcTimeout > 0) {
           ...
           */
          //设置最多连接重试45次，总共有20s*45 = 15分钟的重试时间
    ...
    private synchronized void setupIOstreams() throws InterruptedException {
      ...
      try {
        ...
        while (true) {
          ...
          start();//当建立连接的时，启用接收线程等待服务端的返回数据
          return;
        }
      } catch (IOException e) {
        markClosed(e);
    ...
    public void sendParam(Call call) {
      ...
      try {
        synchronized (this.out) {
          ...
          out.write(data, 0, dataLength);//write the data 向服务器写数据
          out.flush();
        }
      } catch(IOException e) {
        markClosed(e);
  ...
  public Writable call(Writable param, ConnectionId remoteId)  
    ...
    Connection connection = getConnection(remoteId, call);//获取与远端服务器的连接
    connection.sendParam(call);//向服务器发送cell对象                 // send the parameter
   ...
   * pool.  Connections to a given ConnectionId are reused. */
   private Connection getConnection(ConnectionId remoteId,
    ...
    //entire system down.
    connection.setupIOstreams();//真正与服务器端建立连接
    return connection;
  }
