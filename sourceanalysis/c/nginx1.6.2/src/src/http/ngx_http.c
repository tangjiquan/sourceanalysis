...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    ...
    //cscfp指向保存所有ngx_http_core_srv_conf_t结构体指针的servers动态元素的第1个元素
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        //这个操作是递归进行的，如果某个location块下具有其他location，那么它的locations链表也会被排序
        if (ngx_http_init_locations(cf, cscfp[s], clcf) != NGX_OK) {
            return NGX_CONF_ERROR;
        }

        //已经按照location字符串排序过的双向链表，快速地构建静态的二叉查找树，与ngx_http_init_locations方法类似，这个
        //操作也是递归执行的
...
//ngx_http_merge_servers不只是合并了server相关的配置项，同时也会合并location相关的配置项
static char *
    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    cscfp = cmcf->servers.elts;
    //注意，这个ctx是在htt{}块下的全局ngx_http_conf_ctx_t结构体
    ...
    for (s = 0; s < cmcf->servers.nelts; s++) {
        ...
        //如果当前HTTP模块实现了merge_srv_conf，则在调用合并方法
        if (module->merge_srv_conf) {
        	//在这里合并配置项时，saved.srv_conf[ctx_index]参数是当前的HTTP模块在http{}块下由create_srv_conf
        	//方法创建的结构体，而cscfp[s]->ctx->srv_conf[ctx_index]参数则是server{}块下由create_srv_conf方法创建的
        ...
        if (module->merge_loc_conf) {
            ...
            //clcf是server块下ngx_http_core_module模块使用create_loc_conf方法产生的ngx_http_core_loc_cof_t结构体
            //它的locations成员将以双向链表的形式关联到所有当前server{}块下的location块
            clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];
...
//ngx_http_merge_locations方法负责合并location相关的配置项，上面已经将main级别与srv级别做过合并，接下来再次将srv
//级别与loc级别做合并。每一个server块ngx_http_core_loc_conf_t中的locations双向链表会包含所属的全部locations
static char *
    ...
    void **loc_conf, ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    //如果locations链表为空，也就是当前server块下没有location块，则立即返回
    if (locations == NULL) {
        return NGX_CONF_OK;
    }
         ...
         q = ngx_queue_next(q))
    {
        ...
        //如果location后的匹配字符串不一依靠nginx自定义的通配符就可以完全匹配的话，则exact指向当前的location对应的
        //ngx_http_core_loc_conf_t结构体，否则使用inclusive指向该结构体，且exact的优先级别高于inclusive
        ...
        //因为location{}中可以继续嵌套location{}配置块，所有可以继续合并的
