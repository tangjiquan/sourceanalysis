...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    //由事件的data成员取得ngx_connection_t连接。注意，这个连接并不是nginx与客户端的连接，而是nginx与上游服务器间 的连接
    c = ev->data;
    //由连接的data成员取得ngx_http_request_结构体
    r = c->data;

    //由请求的upstream成员取得表示upstream机制的Ngx_http_upstream_t结构体
    u = r->upstream;
    //ngx_http_request_t结构体中的这个connection连接是客户端与Nginx间的连接
    c = r->connection;
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (c->write->timedout) {
    	//将超时错误传递给ngx_http_upstream_next方法，该方法将会根据允许的错误重连策略决策：重新发起连接执行
    	//upstream请求，或者结束upstream请求
    ...
    //header_sent标志位为1时表明上游服务器的响应需要直接转发给客户端，而且此时的Nginx已经把响应包头转发给客户端了
    if (u->header_sent) {
    	//事实上，header_sent为1时一定是已经解析完全部的上游响应包头，并且开始向下发送HTTP的包头了，到此，是不应该继续向上
    	//游发送请求的。所有把writer_event_handler设为任何工作都没有做的ngx_http_upstream_dummy_handler方法
        u->write_event_handler = ngx_http_upstream_dummy_handler;

        //将写事件添加到epoll中
        (void) ngx_handle_write_event(c->write, 0);

        //因为不存在继续发送请求到上游的可能，所有直接返回
        return;
    }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    //这里是直接引用必须分配过内存的pipe指针
    p = u->pipe;

    //设置向下游客户端发送响应的方法为ngx_http_output_filter
    p->output_filter = (ngx_event_pipe_output_filter_pt) ngx_http_output_filter;
    //output_ctx指向当前请求的ngx_http_request_t结构体，这是因为接下来转发包体的方法都只接受ngx_event_pipe_t参数，且只能由
    ...
    //upstream在这里被初始化为nginx与上游服务器间的连接
...
ngx_http_upstream_process_non_buffered_downstream(ngx_http_request_t *r)
{
    ...
    //如果发生超时，那么同样要结束请求，超时时间就是nginx.conf文件中的send_timeout配置项
    if (wev->timedout) {
        c->timedout = 1;
        ngx_connection_error(c, NGX_ETIMEDOUT, "client timed out");
        //结束请求时传递的参数是NGX_HTTP_REQUEST_TIME_OUT
        ngx_http_upstream_finalize_request(r, u, NGX_HTTP_REQUEST_TIME_OUT);
        return;
    }

    //同样调用该方法向客户端发送响应包体，注意传递的第二个参数为1
    ...
    ngx_http_upstream_t *u)
{
    ...
    //这个方法才是真正决定以固定内存块作为缓存时如何转发响应的，注意，传递的第二个参数是0
...
{
	//前文说过，data参数就是ngx_http_upstream_t结构体中的input_filter_ctx,当HTTP模块来实现input_filter方法时，
    ...
    //找到out_bufs链表的末尾，其中cl指向链表中最后一个ngx_chain_t元素的next成员，所以cl最后一定是NULL空指针
    //而ll指向最后一个缓冲区的地址，它用来在后面的代码中向out_bufs链表添加新的缓冲区
    for (cl = u->out_bufs, ll = &u->out_bufs; cl; cl = cl->next) {
        ll = &cl->next;
    }

    //free_bufs指向空闲的ngx_buf_t结构体构成的链表，如果free_bufs此时是空的，那么将会重新由r->pool内存池分配一个ngx_buf_t结构体
    //给cl；如果free_bufs链表不为空，则直接有free_bufs中获取一个ngx_buf_t结构体给cl
    ...
    *ll = cl;
    ...
    //last实际指向本次接收到的包体首地址
    cl->buf->pos = b->last;
    //last向后移动bytes字节，意味着buffer需要保存这次接收的包体
    b->last += bytes;
    //last和pos成员确定了out_bufs链表中每一个缓冲区的包体数据
    cl->buf->last = b->last;
    ...
    ngx_uint_t ft_type)
{
    ...
    if (status) {
        ...
        //只有向这台上游服务器的重试次数tries减为0时，才会真正地调用ngx_http_upstream_finalize_request方法
        //结束请求，否则会再次试图重新与上游服务器交互，这个功能将帮助感兴趣的HTTP模块实现简单的负载均衡机制
        //u->conf->next_upstream实际上是一个32位的错误码组合，表示出现这些错误码时不能直接结束请求，需要
        //向下一台上游服务器再次重发
...
ngx_http_upstream_cleanup(void *data)
{
    ...
    //最终还是调用ngx_http_upstream_finalize_request方法来结束请求
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    //如果使用了TCP连接池实现了free方法，那么调用free方法（如ngx_http_upstream_free_round_robin_peer）
    //释放连接资源
    ...
    //最后还是通过调用HTTP框架提供的ngx_http_finalize_request方法结束请求
    ngx_http_finalize_request(r, rc);
}
