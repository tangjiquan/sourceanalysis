...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    //如果handler方法返回NGX_OK，之后将进入下一个阶段处理，而不会理会当前阶段中是否
    //还有其他的处理方法
    if (rc == NGX_OK) {
    ...
    //如果handler方法返回NGX_DECLINED,那么将进入下一个处理方法，这个处理方法既可能属于当前
    //阶段，也可能属于下一个阶段，注意，返回NGX_OK与NGX_DECLINED
    if (rc == NGX_DECLINED) {
        r->phase_handler++;
        return NGX_AGAIN;
    }
    ...
    //如果handler方法返回NGX_ERROR或者NGX_HTTP_开头的返回码，则调用ngx_http_finalize_request结束请求
