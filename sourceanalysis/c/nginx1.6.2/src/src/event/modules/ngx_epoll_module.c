...
static ngx_command_t  ngx_epoll_commands[] = {
		...
		//在调用epoll_wait时，将由第2和第3个参数告诉Linux内核一次最多可返回多少事件。
		//这个配置项表示调用一次epoll_wait时最多可以返回的事件数，当然，它也会预分配那么多epoll_event结构体
		//用于存储事件
    { ngx_string("epoll_events"),
	  ...
	  //指明在开启异I/O且使用io_setup系统调用初始化异步I/O上下文环境时，初始分配的异步I/O事件个数
...
ngx_event_module_t  ngx_epoll_module_ctx = {
...

    {
		...
		//对应于ngx_event_actions_t中的ebable方法，与add方法一致
		...
		//对于于ngx_event_actions_t中的process_events方法
        ngx_epoll_process_events,        /* process the events */
		//对应于ngx_event_actions_t中的init方法
        ngx_epoll_init,                  /* init the events */
		//对于于ngx_event_actions_t中的done方法
...
ngx_epoll_eventfd_handler(ngx_event_t *ev)
{
    ...
    //获取已经完成的事件数目，并设置到ready中，这个ready是可以大于64的
