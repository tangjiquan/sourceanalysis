...
static ngx_command_t  ngx_event_core_commands[] = {
	...
	//连接池的大小，也就是每个worker进程中支持的TCP最大连接数，它与下面的connections配置项的
	//意义重复
    ...
    { ngx_string("use"),
	  ...
	  //对于epoll事件驱动模式来说，意味着在接收到新的连接事件时，调用accept以尽可能多地接收连接
    ...
    { ngx_string("accept_mutex"),
	  ...
	  //启动accept_mutex负载均衡锁后，延迟accept_mutex_delay毫秒后再试图处理新连接事件
...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    if (delta) {
    	//先处理所有可能超时的事件，如果超时，将event的timeout设为1，并且马上event->handler(ev);
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
    ...
    //计算出需要使用的共享内存的大小。为什么每个统计成员需要使用128字节
    cl = 128;

    size = cl            /* ngx_accept_mutex */
           + cl          /* ngx_connection_counter */
           + cl;         /* ngx_temp_number */
//定义了NGX_STAT_STUB宏后才会统计述6个原子变量
#if (NGX_STAT_STUB)
