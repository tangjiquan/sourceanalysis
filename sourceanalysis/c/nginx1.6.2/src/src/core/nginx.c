...
main(int argc, char *const *argv)
{
    ...
    //完成操作系统的一些信息的获取，如内存页面的大小，所有的这些资源将保存到对应的全局变量中，方便访问
   ...
   //通过环境变量nginx完成socket的继承，继承来的socket将会放到init_cycle的listening数组中。在NGINX环境变量中，
    //每个socket中间用冒号或分号隔开。
    //完成继承同时设置全局变量ngx_inherited为1。
    ...
    if (ngx_process == NGX_PROCESS_SINGLE) {
    	...
    	/**
    	 * 进程处理
    	 * 主进程master产生多个工作进程worker
    	 * 这里将做各个模块进程初始化，监听，接受，请求处理，还有信号等
    	 */
...
//将命令行参数保存到ngx_os_argv,ngx_argc, ngx_argv，备份一下，方便后面的初始化工作能够随时获得命令行参数
static ngx_int_t
