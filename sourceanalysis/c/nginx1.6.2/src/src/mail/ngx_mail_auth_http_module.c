...
struct ngx_mail_auth_http_ctx_s {
	//request缓冲区保存着发往认证服务器的请求，它是根据解析客户端请求得到的ngx_mail_session_t，使用ngx_mail_auth_http_create_request方法构造出
	//的内存缓冲区，这里请求是一种类HTTP的请求
    ngx_buf_t                      *request;
    ...
    //解析来自认证服务器类HTTP的响应行，头部的方法，默认为ngx_mail_auth_http_ignore_status_line方法
    ngx_mail_auth_http_handler_pt   handler;

    //在使用状态机制解析认证服务器返回的类HTTP响应时，使用state表示解析状态
    ngx_uint_t                      state;

    //ngx_mail_auth_http_parse_header_line方法负责解析认证服务器发来的响应类HTTP的头部，一下4个成员用于解析响应头部
    ...
    //错误信息构成的字符串
    ngx_str_t                       errmsg;
    //错误码构成的字符串。如果认证服务器返回的头部里有Auth-Error-Code，那么将设置到errcode中，errmsg和errcode在
    //发生错误时会直接将其作为响应发送给客户端
    ngx_str_t                       errcode;

    //认证服务器返回Auth-Wait头部时会带的时间戳将会被设到sleep成员中，而nginx等待的时间也将由sleep维护，当
    //sleep降低为0时将会设置quit标志位为1，表示请求非正常结束，把错误码返回给用户
    time_t                          sleep;

    //用于邮件认证的独立内存池，它的初始化大小为2KB
    ngx_pool_t                     *pool;
};
...
ngx_mail_auth_http_write_handler(ngx_event_t *wev)
{
    ...
    c = wev->data;
    //连接的data成员指向ngx_mail_session_t结构体
    s = c->data;
    ...
    //计算还剩下多少字节的请求没有发送，pos和last之间的内容就是待发送的请求
    ...
    if (n > 0) {
        ...
        if (n == size) {
        	//将nginx于认证服务器间连接的写事件回调方法设为任何事情都不做的ngx_mail_auth_http_dummy_handler方法
