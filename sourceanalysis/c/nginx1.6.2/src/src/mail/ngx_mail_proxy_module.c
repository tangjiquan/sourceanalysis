...
ngx_mail_proxy_init(ngx_mail_session_t *s, ngx_addr_t *peer)
{
    ...
    //设置nginx与客户端间连接事件的回调方法为不会读取内容的ngx_mail_proxy_block_read方法，因为
    //当前阶段nginx不会与客户端交互
    s->connection->read->handler = ngx_mail_proxy_block_read;
    //设置nginx与上游间的连接写事件回调方法为什么事都不做的ngx_mail_proxy_dummy_handler方法，
    //这意味着接下来向上游发送TCP流时，将不再通过epoll这个事件框架来调度
    p->upstream.connection->write->handler = ngx_mail_proxy_dummy_handler;

    pcf = ngx_mail_get_module_srv_conf(s, ngx_mail_proxy_module);

    //建立nginx与邮件服务器间的内存缓冲区，缓冲区大小由nginx.conf文件中的proxy_buffer配合决定
...
ngx_mail_proxy_pop3_handler(ngx_event_t *rev)
{
    ...
    case ngx_pop3_passwd:
    	//在收到服务器返回的密码验证通过信息后，将nginx与下游客户端间，nginx与上游邮件服务器间的TCP
    	//连接上读/写事件的回调方法都设置为ngx_mail_proxy_handler方法
    ...
    //向上游的邮件服务器发送验证信息，这里向邮件服务器发送TCP流与本书的其他章节都不同，它不再通过epoll
    //检测到TCP连接上出现可写事件而触发，事实上，它是有连接上出现的可读事件出发的，因为读取到了邮件服务器的
    //消息，才向邮件服务器发送消息，之所以可以这么做的一个原因在于，当前阶段发送的TCP消息包都非常短小
...
ngx_mail_proxy_handler(ngx_event_t *ev)
{
    ...
    //size变量具有两种含义：在读取消息时，size表示recv方法中空闲缓冲区的大小，在发送消息时，表示将要发送的消息的长度
    size_t                  size;
    //send或者recv方法的返回值
    ssize_t                 n;
    ...
    ngx_uint_t              do_write;
    //每次透传TCP，上，下游的客户端与邮件服务器之前必然有一个负责提供消息，另一个负责接收nginx转发的消息，
    //src用来表示nginx与提供消息一方间的连接，而dst表示nginx与接收消息一方之间的连接
    ngx_connection_t       *c, *src, *dst;
    ngx_mail_session_t     *s;
    ngx_mail_proxy_conf_t  *pcf;

    //事件ev既可能属于nginx与下游间的连接，也有可能属于nginx与上游间的连接，此时无法判断连接c究竟是来自上游还是下游
    c = ev->data;
    //无论是上游还是下游连接上，ngx_connection_t结构体的data成员都指向ngx_mail_session_t结构体
    s = c->data;
    ...
    if (c == s->connection) {
        ...
        if (ev->write) {
        	//当下游可读事件被触发，意味着本次nginx将负责把接收自上游的缓存消息发送给下游
    ...
    //当前触发事件的write标志将决定do_write本次是发送消息还是接收消息
    ...
    //进入向dst连接发送消息或者由src连接上接收消息的循环，直到套接字上暂无可读或可写事件方退出
    for ( ;; ) {
        ...
        if (size && src->read->ready) {
            ...
            //如果没有读取到内容，或者对方主动关闭TCP连接，则跳出循环
            if (n == NGX_AGAIN || n == 0) {
                break;
            }

            if (n > 0) {
            	//如果读取到了消息，则应试图在本次ngx_mail_proxy_handler方法的执行中将它立即发送出去
                do_write = 1;
                //更新消息缓冲区
                b->last += n;
    ...
    //下面将会把nginx与上下游TCP连接上的4个读/写事件再次添加到epoll中监控
